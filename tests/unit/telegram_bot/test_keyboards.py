...
def test_format_price_preset(min_price, max_price, expected):
    # Проверяет форматирование ценовых диапазонов с разделением тысяч пробелами
    # и символом рубля.
    assert _format_price_preset(min_price, max_price) == expected


def test_build_start_set_price_filter_kbd_has_preset_buttons():
    # Проверяет, что число рядов клавиатуры равно числу ценовых пресетов плюс
    # один ряд отмены.
...
def test_build_start_set_price_filter_kbd_callback_data():
    # Проверяет отдельную кнопку для каждого ценового пресета и передачу обеих
    # границ в callback.
    kbd = build_start_set_price_filter_kbd()
...
def test_build_start_set_price_filter_kbd_cancel_button():
    # Проверяет, что последний ряд клавиатуры содержит callback отмены
    # настройки ценового фильтра.
    kbd = build_start_set_price_filter_kbd()
...
def test_preset_price_filter_cb_roundtrip():
    # Проверяет сохранение минимальной и максимальной цены после упаковки и
    # распаковки callback пресета.
