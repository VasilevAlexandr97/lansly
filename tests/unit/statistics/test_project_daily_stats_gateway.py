...
def test_bucket_metric_filters_on_price_bounds(
    ...
    # Проверяет, что SQL метрик ценовых диапазонов содержит FILTER с заданными
    # границами цены.
...
def test_count_metric_is_plain_count(gateway: SAProjectDailyStatsGateway):
    # Проверяет, что метрика количества проектов выражается через count(*).
...
def test_sum_price_metric_coalesces_zero(gateway: SAProjectDailyStatsGateway):
    # Проверяет использование coalesce с нулём в SQL суммы цен проектов.
    compiled = gateway._metric_expression(
...
def test_min_max_metrics_aggregate_price(gateway: SAProjectDailyStatsGateway):
    # Проверяет использование min и max по полю цены в соответствующих метриках
    # проектов.
    assert (
        "min(projects.price)"
...
def test_unknown_metric_raises_value_error(
    ...
    # Проверяет, что запрос выражения для неизвестной метрики вызывает
    # ValueError.
...
def test_all_metrics_have_supported_expressions(
    ...
    # Проверяет наличие девяти метрик проектов и возможность построить
    # SQL-выражение для каждой из них.
...
def test_rows_from_values_builds_rows_in_metric_order(
    ...
    # Проверяет преобразование значений в строки метрик с сохранением порядка,
    # даты и измерения.
...
def test_rows_from_values_skips_none_for_nullable_metrics(
    ...
    # Проверяет исключение метрик минимальной и максимальной цены из
    # результата, когда их значения равны None.
    metrics = SAProjectDailyStatsGateway._ALL_METRICS
...
def test_rows_from_values_without_metrics_returns_empty(
    ...
    # Проверяет возврат пустого списка строк при отсутствии метрик и значений.
    assert (
...
def test_day_range_uses_utc_day_bounds(gateway: SAProjectDailyStatsGateway):
    # Проверяет, что границы дня проектов соответствуют полуночи указанной и
...
def test_source_dimension_formats_source_prefix():
    # Проверяет формирование измерения площадки в формате source:kwork.
    assert source_dimension("kwork") == "source:kwork"


def test_category_dimension_formats_source_and_external_id():
    # Проверяет формирование измерения категории из площадки и внешнего ID в
    # формате cat:kwork:23.
    assert category_dimension("kwork", "23") == "cat:kwork:23"


def test_category_dimension_uses_uncategorized_when_no_external_id():
    # Проверяет использование суффикса none в измерении категории без внешнего
    # ID.
